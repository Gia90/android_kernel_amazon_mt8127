...
#ifdef OEM_ANDROID
#define CONFIG_ANDROID_BCMDHD_FW_PATH "brcm/bcm43569a2-firmware.bin"
#define CONFIG_ANDROID_BCMDHD_NVRAM_PATH "brcm/bcm43569a2.nvm"
#endif /* OEM_ANDROID */
...
#else
		req->urb->transfer_flags |= URB_QUEUE_BULK | URB_RX_REQ_MODE1_ENABLE;
#endif
            ...
            pm_message_t message)
{
        ...
        if (usbos_info->cbarg) {
		dhd_dbus_set_suspend_state(usbos_info->cbarg, 1);
                usbos_info->cbs->set_wow(usbos_info->cbarg, wlan_if, 1);
	}
#endif
...
static int dbus_usbos_resume(struct usb_interface *intf)
{
        ...
        if (usbos_info->cbarg) {
		dhd_dbus_set_suspend_state(usbos_info->cbarg, 0);
	}
#endif
...
#ifdef CONFIG_WLAN_STR_SUPPORT
        if (usbos_info->cbarg) {
                usbos_info->cbs->set_wow(usbos_info->cbarg, wlan_if, 0);
        }
#endif
...
#ifdef CONFIG_WLAN_STR_SUPPORT
        DBUSERR(("%s Device reset resumed for STR\n", __FUNCTION__));
        dbus_usbos_resume(intf);
#else
...
#ifdef CONFIG_WLAN_AARCH64_SUPPORT
        DBUSERR(("%s: disconnect to USB controller.\n", __FUNCTION__));
#endif

	if (probe_usb_init_data) {
		usbos_info = (usbos_info_t *) probe_usb_init_data->usbos_info;
		if (usbos_info) {
			if (disconnect_cb && disc_arg && __sync_bool_compare_and_swap(&probe_usb_init_data->disc_cb_done, FALSE, TRUE)) {
...
#ifdef CONFIG_WLAN_STR_SUPPORT
	if(!probe_usb_init_data || !usbos_info || FALSE == probe_usb_init_data->disc_cb_done)
	{
		DBUSERR(("%s: g_probe_info=%p,probe_usb_init_data=%p,usbos_info=%p,disc_cb_done=%s,dereged=%s,disc_cb=%p,disc_arg=%p\n",
			__FUNCTION__,
			&g_probe_info,
			probe_usb_init_data,
			usbos_info,
			(probe_usb_init_data->disc_cb_done == TRUE) ? "TRUE" : "FALSE",
			(probe_usb_init_data->dereged == TRUE) ? "TRUE" : "FALSE",
			disconnect_cb,
			disc_arg));
		DBUSERR(("dumping g_probe_info:\n"));
		DBUSERR(("	usbos_info=%p\n",    g_probe_info.usbos_info));
		DBUSERR(("	usb       =%p\n",    g_probe_info.usb));
		DBUSERR(("	vid       =%d\n",    g_probe_info.vid));
		DBUSERR(("	pid       =%d\n",    g_probe_info.pid));
		DBUSERR(("	dereged   =%d\n",    g_probe_info.dereged));
		DBUSERR(("	disc_cb_done=%d\n",  g_probe_info.disc_cb_done));
		DBUSERR(("	device_speed=%d\n",  g_probe_info.device_speed));
		DBUSERR(("	suspend_state=%d\n", g_probe_info.suspend_state));
		DBUSERR(("	intf=%p\n",          g_probe_info.intf));
		BUG_ON(TRUE);
	}
#endif
...
dbus_bus_osl_deregister()
{
	...
	if (disconnect_cb && disc_arg && __sync_bool_compare_and_swap(&g_probe_info.disc_cb_done, FALSE, TRUE)) {
		disconnect_cb(disc_arg);
		disc_arg = NULL;
	}
