...
#define CMD_DYN_BW_MODE		"DYN_BW_MODE"
/* fosmod_wifi_enhancements begin */
#define CMD_TXCHAIN		"TXCHAIN"
#define CMD_GET_PWRTHROTTLE_MASK	"GET_PWRTHROTTLE_MASK"
#define CMD_SET_PWRTHROTTLE_MASK	"SET_PWRTHROTTLE_MASK"
#define CMD_GET_PWRTHROTTLE		"GET_PWRTHROTTLE"
#define CMD_SET_PWRTHROTTLE		"SET_PWRTHROTTLE"
#define CMD_GET_DUTYCYCLE_THERMAL	"GET_DUTYCYCLE_THERMAL"
#define CMD_SET_DUTYCYCLE_THERMAL	"SET_DUTYCYCLE_THERMAL"
/* fosmod_wifi_enhancements end */
...
#ifdef WSS
#define CMD_FEA_COMMANDS	"FEACMD"
#endif /* WSS */
...
#ifdef WSS
int do_bypass_probe=0;
int amz_test_ssid_extra=0;

int wl_android_fea_commands(struct net_device *ndev, char *data,
		char *command, int total_len)
{
	...
	argc = sscanf(command, "%d %d", &subcmd,&val);
	switch (subcmd) {
	case VENDOR_NL80211_SUBCMD_PROBE_NO_SSID_CHECK:
		if(argc==2){
			wl_add_remove_eventmsg(ndev,WLC_E_PROBREQ_MSG,val?true:false);
			do_bypass_probe=val?1:0;
			probe_req=val;

			err = wldev_iovar_setbuf(ndev, "probreq_host", (void *)&probe_req,
				sizeof(probe_req),iobuf, WLC_IOCTL_SMLEN, NULL);
			if (err != BCME_OK && err != BCME_UNSUPPORTED) {
				WL_ERR(("ERR %d set prob to host %d\n", err, probe_req));
			}
		}else{
			err = wldev_iovar_getbuf(ndev, "probreq_host", (void *)&probe_req,
				sizeof(probe_req),
				iobuf, WLC_IOCTL_SMLEN, NULL);
		}
		bytes_written = snprintf(command, total_len, "New %d res %d,fw %d\n",do_bypass_probe,err,probe_req);
		err = bytes_written;
		break;
	case VENDOR_NL80211_SUBCMD_TEST_ADD_EXTRA_SSID:
		if(argc==2){
			amz_test_ssid_extra=val;
		}
		bytes_written = snprintf(command, total_len, "SSID Extra %d\n",amz_test_ssid_extra);
		err = bytes_written;
		break;
	default:
		break;
...
static int
wl_dyn_bw_mode(struct net_device *dev, char *cmd, int total_len)
{
	char *ptr = "dyn_bw_mode";
	int err = BCME_OK;
	int flag = 0;
	char buf[64];
	int buf_len;
	...
	wl_reassoc_params_t rparam;
	u32 chan = 0, chspec = 0;

	DHD_INFO(("%s: DYN_BW_MODE CMD=%s\n", __FUNCTION__, cmd));

	/* String should look like below */
	/*   DYN_BW_MODE [0/1]  ex: DYN_BW_MODE 1 */

	/* Get dynamic bandwidth mode */
	if (sscanf(cmd, "%d", &flag) != 1) {
		DHD_ERROR(("%s: Failed to get Parameter\n", __FUNCTION__));
		return err;
	}

	/* Set dynamic bandwidth mode, always */
	memset(buf, 0, sizeof(buf));
	len = strlen(CMD_DYN_BW_MODE);
	strncpy(buf, ptr, len);
	buf[len] = '\0';
	buf_len = len+1;
	...
	ptr = buf + buf_len;
	memcpy(ptr, &param, sizeof(struct param));
	buf_len += sizeof(struct param);

	err = wldev_ioctl(dev, WLC_SET_VAR, buf, buf_len+1, TRUE);
	if (err) {
		DHD_ERROR(("%s: Failed to set dyn_bw_mode=%d, error = %d\n",
			__FUNCTION__, flag, err));
		return err;
	}

	/*
	 * Ignore reassociation, if
	 *  - STA is NOT currently associated to AP
	 *  - STA associated channel is in 2.4GHz
	 *  - STA associated bandwidth is same as the expected bandwidth
	 */

	/* Get BSSID. Valid BSSID is returned only when STA is associated */
	bzero(&rparam.bssid, sizeof(rparam.bssid));
	if ((err = wldev_ioctl(dev, WLC_GET_BSSID, &rparam.bssid, ETHER_ADDR_LEN, false)) == 0) {
		DHD_INFO(("%s: STA BSSID is " MACDBG "\n", __FUNCTION__, MAC2STRDBG(rparam.bssid.octet)));
	} else {
		WL_ERR(("%s: STA is not associated to AP - %d\n", __FUNCTION__, err));
		return err;
	}

	/* Get chanspec and convert to channel/bandwidth format */
	err = wldev_iovar_getint(dev, "chanspec", (s32 *)&chspec);
	if (err != BCME_OK) {
		WL_ERR(("%s: chanspec iovar error - %d\n", __FUNCTION__, err));
		return err;
	}
	chan = wf_chspec_ctlchan(chspec);

	/* Verify associated bandwidth is same as the requested */
	if (flag) {
		if (CHSPEC_IS20(chspec)) return 0;
		snprintf(buf, 64, "%d/20", chan);
	} else {
		if (CHSPEC_IS2G(chspec) || (CHSPEC_IS80(chspec))) return 0;
		snprintf(buf, 64, "%d/80", chan);
	}

	DHD_ERROR(("%s: channel=%s\n", __FUNCTION__, buf));

	/* Reassociate, now */
	rparam.chanspec_num = 1;
	rparam.chanspec_list[0] = wf_chspec_aton(buf);
	err = wldev_ioctl(dev, WLC_REASSOC, &rparam, sizeof(rparam), true);
	if (err < 0) {
		WL_ERR(("%s: reassoc iovar error %d\n", __FUNCTION__, err));
		return err;
	}
...
/* fosmod_wifi_enhancements begin */
/* implement wl_android_set_txchain */
static int
wl_android_set_txchain(struct net_device *dev, char *command, int total_len)
{
	char *pcmd = command;
	char *str = NULL;
	int txchain;
	...
	WL_DBG(("%s: about to set txchain, command = %s \n", __FUNCTION__, command));
	...
	if (txchain > 3 || txchain <= 0) {
		WL_ERR(("Invalid parameter: valid txchain are 1, 2, 3\n"));
		return -EINVAL;
	}
...
/* implement wl_android_get_pwrthrottle_mask */
static int
wl_android_get_pwrthrottle_mask(struct net_device *dev, char *command, int total_len)
{
	int err;
	int bytes_written = 0;
	int status;
	err = wldev_iovar_getint(dev, "pwrthrottle_mask", &status);

	if (err) {
		DHD_ERROR(("%s: get pwrthrottle_mask status failed: code %d\n", __func__, err));
		return -1;
	}
	else {
		DHD_INFO(("%s: get pwrthrottle_mask status %d\n", __func__, status));
	}

	bytes_written = snprintf(command, total_len, "%s %d", CMD_GET_PWRTHROTTLE_MASK, status);

	return bytes_written;
}

/* implement wl_android_set_pwrthrottle_mask */
static int
wl_android_set_pwrthrottle_mask(struct net_device *net, int pwrthrottle_mask)
{
	int err = 0;
	WL_DBG(("%s: about to set pwrthrottle_mask \n", __FUNCTION__));
	if (pwrthrottle_mask > 3 || pwrthrottle_mask < 0) {
		WL_ERR(("Invalid parameter: valid pwrthrottle_mask values are 0, 1, 2, 3\n"));
		return -EINVAL;
	}
	err = wldev_iovar_setint(net, "pwrthrottle_mask", pwrthrottle_mask);
	return err;
}

/* implement wl_android_get_pwrthrottle */
static int
wl_android_get_pwrthrottle(struct net_device *dev, char *command, int total_len)
{
	int err;
	int bytes_written = 0;
	int status;
	err = wldev_iovar_getint(dev, "pwrthrottle", &status);

	if (err) {
		DHD_ERROR(("%s: get pwrthrottle status failed: code %d\n", __func__, err));
		return -1;
	}
	else {
		DHD_INFO(("%s: get pwrthrottle status %d\n", __func__, status));
	}

	bytes_written = snprintf(command, total_len, "%s %d", CMD_GET_PWRTHROTTLE, status);

	return bytes_written;
}

/* implement wl_android_set_pwrthrottle */
static int
wl_android_set_pwrthrottle(struct net_device *net, int pwrthrottle_enable)
{
	int err = 0;
	WL_DBG(("%s: about to set pwrthrottle \n", __FUNCTION__));
	if (pwrthrottle_enable != 0 && pwrthrottle_enable != 1) {
		WL_ERR(("Invalid parameter: valid values are 0 to disable or 1 to enable tx throttle\n"));
		return -EINVAL;
	}
	err = wldev_iovar_setint(net, "pwrthrottle", pwrthrottle_enable);
	return err;
}

/* implement wl_android_get_dutycycle_thermal */
static int
wl_android_get_dutycycle_thermal(struct net_device *dev, char *command, int total_len)
{
	int err;
	int bytes_written = 0;
	int status;
	err = wldev_iovar_getint(dev, "dutycycle_thermal", &status);

	if (err) {
		DHD_ERROR(("%s: get dutycycle_thermal status failed: code %d\n", __func__, err));
		return -1;
	}
	else {
		DHD_INFO(("%s: get dutycycle_thermal status %d\n", __func__, status));
	}

	bytes_written = snprintf(command, total_len, "%s %d", CMD_GET_DUTYCYCLE_THERMAL, status);

	return bytes_written;
}

/* implement wl_android_set_dutycycle_thermal */
static int
wl_android_set_dutycycle_thermal(struct net_device *dev, char *command, int total_len)
{
	char *pcmd = command;
	char *str = NULL;
	int dutycycle_thermal;
	int err = 0;

	WL_DBG(("%s: about to set dutycycle_thermal, command = %s \n", __FUNCTION__, command));

	/* acquire parameters */
	/* drop comand */
	str = bcmstrtok(&pcmd, " ", NULL);

	/* acquire dutycle_thermal value */
	str = bcmstrtok(&pcmd, " ", NULL);
	if (!str) {
		WL_ERR(("Invalid parameter : %s\n", pcmd));
		return -EINVAL;
	}
	dutycycle_thermal = bcm_atoi(str);

	if (dutycycle_thermal < 0 || dutycycle_thermal > 100) {
		WL_ERR(("Invalid parameter: valid dutycycle_thermal value is an int from 0 to 100\n"));
		return -EINVAL;
	}
	err = wldev_iovar_setint(dev, "dutycycle_thermal", dutycycle_thermal);
	return err;
}

/* implement wl_android_get_pm */
static int
wl_android_get_pm(struct net_device *dev, char *command, int total_len)
{
	...
	if (err) {
		DHD_ERROR(("%s: get pm mode failed: code %d\n", __func__, err));
		return -1;
	}
	else {
		DHD_INFO(("%s: get pm mode %d\n", __func__, pm));
	}

	bytes_written = snprintf(command, total_len, "%s %d", CMD_GET_PM, pm);

	return bytes_written;
}

/* implement wl_android_set_pm */
static int
wl_android_set_pm(struct net_device *dev, char *command, int total_len)
{
	...
	DHD_ERROR(("%s: about to set pm mode = %s \n", __FUNCTION__, command));

	/* acquire parameters */
	/* drop comand */
	str = bcmstrtok(&pcmd, " ", NULL);

	/* acquire pm mode value */
	str = bcmstrtok(&pcmd, " ", NULL);
	if (!str) {
		DHD_ERROR(("Invalid parameter : %s\n", pcmd));
		return -EINVAL;
	}
	pm_mode = bcm_atoi(str);

	if (pm_mode != 0 && pm_mode != 2) {
		DHD_ERROR(("Invalid parameter: valid pm_mode are 0 or 2.\n"));
		return -EINVAL;
	}

	err = wldev_ioctl(dev, WLC_SET_PM, &pm_mode, sizeof(pm_mode), true);
	if (err) {
		DHD_ERROR(("wl_android_set_pm err: %d\n", err));
	} else {
		wl_cfg80211_update_power_mode(dev);
	}
	return err;
...
wl_handle_private_cmd(struct net_device *net, char *command, u32 cmd_len)
{
	...
	/* CUSTOMER_SET_COUNTRY feature is define for only GGSM model */
		...
		bytes_written = wl_android_wbtext_weight_config(net, data,
				command, priv_cmd.total_len);
	}
		...
		bytes_written = wl_android_wbtext_table_config(net, data,
				command, priv_cmd.total_len);
	}
#ifdef SET_RPS_CPUS
...
#endif
#ifdef WSS
	else if (strnicmp(command, CMD_FEA_COMMANDS,
		strlen(CMD_FEA_COMMANDS)) == 0) {
		char *data = (command + strlen(CMD_FEA_COMMANDS) + 1);
		bytes_written = wl_android_fea_commands(net, command,data,
			priv_cmd.total_len);
	}
#endif /* WSS */
	else if (strnicmp(command, CMD_DYN_BW_MODE, strlen(CMD_DYN_BW_MODE)) == 0) {
		//DHD_ERROR(("wpa_cli IFNAME=wlan0 DRIVER %s\n", command));
		bytes_written = wl_dyn_bw_mode(net, (command + strlen(CMD_DYN_BW_MODE) +1),
					(priv_cmd.total_len - (strlen(CMD_DYN_BW_MODE)+1)));
	}
	/* fosmod_wifi_enhancements begin */
	else if (strnicmp(command, CMD_TXCHAIN, strlen(CMD_TXCHAIN)) == 0) {
		bytes_written = wl_android_set_txchain(net, command, priv_cmd.total_len);
	}
	else if (strnicmp(command, CMD_GET_PWRTHROTTLE_MASK, strlen(CMD_GET_PWRTHROTTLE_MASK)) == 0) {
		bytes_written = wl_android_get_pwrthrottle_mask(net, command, priv_cmd.total_len);
	}
	else if (strnicmp(command, CMD_SET_PWRTHROTTLE_MASK, strlen(CMD_SET_PWRTHROTTLE_MASK)) == 0) {
		int pwrthrottle_mask = *(command + strlen(CMD_SET_PWRTHROTTLE_MASK) + 1) - '0';
		bytes_written = wl_android_set_pwrthrottle_mask(net, pwrthrottle_mask);
	}
	else if (strnicmp(command, CMD_GET_PWRTHROTTLE, strlen(CMD_GET_PWRTHROTTLE)) == 0) {
		bytes_written = wl_android_get_pwrthrottle(net, command, priv_cmd.total_len);
	}
	else if (strnicmp(command, CMD_SET_PWRTHROTTLE, strlen(CMD_SET_PWRTHROTTLE)) == 0) {
		int pwrthrottle_enable = *(command + strlen(CMD_SET_PWRTHROTTLE) + 1) - '0';
		bytes_written = wl_android_set_pwrthrottle(net, pwrthrottle_enable);
	}
	else if (strnicmp(command, CMD_GET_DUTYCYCLE_THERMAL, strlen(CMD_GET_DUTYCYCLE_THERMAL)) == 0) {
		bytes_written = wl_android_get_dutycycle_thermal(net, command, priv_cmd.total_len);
	}
	else if (strnicmp(command, CMD_SET_DUTYCYCLE_THERMAL, strlen(CMD_SET_DUTYCYCLE_THERMAL)) == 0) {
		bytes_written = wl_android_set_dutycycle_thermal(net, command, priv_cmd.total_len);
	}
