...
static void dbus_if_netdev_wl(void *handle, int idx, int cmd);
static void dbus_if_netdev_running(void *handle, int idx);
static void dbus_if_set_wow(void *handle, int idx, int val);
#endif
...
#ifdef CONFIG_WLAN_STR_SUPPORT
static void
dbus_if_netdev_running(void *handle, int idx)
{
       dbus_info_t *dbus_info = (dbus_info_t *) handle;
        ...
        if (dbus_info->cbs && dbus_info->cbs->netdev_running)
                dbus_info->cbs->netdev_running(dbus_info->cbarg, idx);
}

static void
dbus_if_netdev_wl(void *handle, int idx, int cmd)
{
       dbus_info_t *dbus_info = (dbus_info_t *) handle;
        ...
        if (dbus_info->cbs && dbus_info->cbs->netdev_wl)
                dbus_info->cbs->netdev_wl(dbus_info->cbarg, idx, cmd);
}

static void
dbus_if_set_wow(void *handle, int idx, int val)
{
       dbus_info_t *dbus_info = (dbus_info_t *) handle;
        ...
        if (dbus_info->cbs && dbus_info->cbs->set_wow)
                dbus_info->cbs->set_wow(dbus_info->cbarg, idx, val);
}
#endif
