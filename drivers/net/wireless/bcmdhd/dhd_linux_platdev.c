...
static int idme_get_mac_addr(unsigned char *mac_addr)
{
	...
	if (IS_ERR(f)) {
                DHD_ERROR(("%s: wrong mac addr file\n", __FUNCTION__));
		return -1;
	}
...
//module_platform_driver(wifi_platform_dev_driver);
...
static int wifi_platdev_match(struct device *dev, void *data)
{
	...
	if (strncmp(pdev->name, name, strlen(name)) == 0) {
...
#ifndef CONFIG_WLAN_AARCH64_SUPPORT
	err = wifi_platform_set_power(adapter, TRUE, WIFI_TURNON_DELAY);
#endif
	if (err) {
		DHD_ERROR(("%s: failed to power up %s\n", __func__, adapter->name));
