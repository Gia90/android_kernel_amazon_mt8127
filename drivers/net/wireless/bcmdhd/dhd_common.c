...
/* Common Definitions relocated from wl_iw.h */
...
void
dhd_wowl_set(dhd_pub_t * dhd)
{
	int retcode;

        retcode = dhd_wl_ioctl_set_intiovar(dhd, "wowl",
                2, WLC_SET_VAR, TRUE, 0);

        retcode = retcode >= 0 ? 0 : retcode;
        if (retcode)
                DHD_TRACE(("%s: failed to set wowl, retcode = %d\n",
                        __FUNCTION__, retcode));
        else
                DHD_TRACE(("%s: successfully set wowl\n",
                        __FUNCTION__));

        retcode = dhd_wl_ioctl_set_intiovar(dhd, "wowl_gpio", 14, WLC_SET_VAR, TRUE, 0);

	retcode = retcode >= 0 ? 0 : retcode;
        if (retcode)
                DHD_TRACE(("%s: failed to set wowl_gpio, retcode = %d\n",
                        __FUNCTION__, retcode));
        else
                DHD_TRACE(("%s: successfully set wowl_gpio\n",
                        __FUNCTION__));

        retcode =  dhd_wl_ioctl_set_intiovar(dhd, "wowl_gpiopol", 0, WLC_SET_VAR, TRUE, 0);

	retcode = retcode >= 0 ? 0 : retcode;
        if (retcode)
                DHD_TRACE(("%s: failed to set wowl_gpiopol, retcode = %d\n",
                        __FUNCTION__, retcode));
        else
                DHD_TRACE(("%s: successfully set wowl_gpiopol\n",
                        __FUNCTION__));
}
