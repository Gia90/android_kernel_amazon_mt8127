 ...
 *
 * $Id: wl_cfg80211.c 619565 2016-02-17 09:07:30Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
#ifdef WSS
#include <bcmutils.h>
extern int amz_test_ssid_extra;
extern int do_bypass_probe;
#endif /* WSS */
	...
	if (cfg->p2p_supported && (wlif_type != -1)) {
		...
		if (timeout > 0 && !wl_get_p2p_status(cfg, IF_ADDING) && cfg->if_event_info.valid) {
			...
			(cfg->p2p_ProbeReq_msg_DoNotDisable == 0)) {
				if ( wl_add_remove_eventmsg(primary_ndev,
...
#ifdef CONFIG_WLAN_STR_SUPPORT
		/* Flush the stale keys in FW for WPA-PSK case */
		if(val==WPA_AUTH_PSK) {
			err = wldev_iovar_getint(dev, "gtk_key_info", &val);
			WL_DBG(("gtk_key_info IOVAR Output = %d\n", err));
			val = WPA_AUTH_PSK;
		}
#endif /* STR support */
	...
	struct cfg80211_connect_params *sme)
{
	...
	*/
	...
	if (cfg->rcc_enabled) {
		WL_ERR(("Connecting with" MACDBG " with rcc channels \n\n",
			MAC2STRDBG((u8*)(&ext_join_params->assoc.bssid))));
	} else {
		WL_ERR(("Connecting with" MACDBG " channel=%d\n\n",
...
#ifdef WSS
void
wlll_hexdump(uchar *buf, uint nbytes, uint saddr)
{
	char line[256];
	char* p;
	uint i;
...
#ifdef WSS
			if (do_bypass_probe) {
				mgmt_frame = (wl_mgmt_frame_t *) kzalloc(WL_WIFI_MGMT_FRAME_SIZE, GFP_KERNEL);
				if (mgmt_frame == NULL)
				{
				...
				/* Add the packet Id */
				mgmt_frame->packetId = *cookie;
				mgmt_frame->fc= FC_PROBE_RESP;
				/* Add BSSID */
				memcpy(&mgmt_frame->da, &mgmt->da[0], ETHER_ADDR_LEN);
				mgmt_frame->len= len - DOT11_MGMT_HDR_LEN;
				memcpy(mgmt_frame->data, &buf[DOT11_MGMT_HDR_LEN], mgmt_frame->len);
				wlll_hexdump((uchar*)buf,32,0);
				wlll_hexdump(mgmt_frame->data,32,0);
				err=wldev_iovar_setbuf_bsscfg(dev, "wifimgmt", mgmt_frame, WL_WIFI_MGMT_FRAME_SIZE-ACTION_FRAME_SIZE+mgmt_frame->len,
						cfg->ioctl_buf,WLC_IOCTL_MAXLEN, bssidx, &cfg->ioctl_buf_sync);

				WL_ERR(("action frame %d,len %d,ret %d\n", mgmt_frame->packetId,(int)len,err));
	...
	u32 dev_role, s32 bssidx)
{
	...
	if ((err = wldev_iovar_setint(dev, "closednet", info->hidden_ssid)) < 0)
...
/** ACOS_MOD_BEGIN { wifi_p2p_passwordless_go_support } *i*/
#ifdef PASSWORDLESS_GO_SUPPORT
		if (!is_bssup) {
#else
		if (!is_bssup && (ies->wpa2_ie != NULL)) {
#endif
/** ACOS_MOD_END { wifi_p2p_passwordless_go_support } **/
			printk("jw wl_cfg80211_bcn_bringup_ap\n");
	...
	struct cfg80211_beacon_data *info)
{
	...
	if ((ssid_ie = bcm_parse_tlvs((u8 *)&info->head[ie_offset],
		...
		if ((err = wldev_iovar_setint(dev, "closednet", ssid_ie->len == 0)) < 0) {
			WL_ERR(("failed to set hidden : %d\n", err));
			goto fail;
		}
	}
...
#else
	if (!wl_get_drv_status(cfg, SCANNING, ndev)) {
#ifdef CONFIG_WLAN_AARCH64_SUPPORT
		WL_DBG(("scan is not ready \n"));
#else
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		...
		if (do_bypass_probe) {
			bcm_tlv_t *ssid_ie;
			struct dot11_management_header * hdr = NULL;
//			char ssidbuf[32];

			mgmt_frame = (u8 *)(u8 *)((wl_event_rx_frame_data_t *)rxframe + 1);;
			...
			/* find the SSID */
			if ((ssid_ie = bcm_parse_tlvs(&mgmt_frame[DOT11_MGMT_HDR_LEN],
				prbreq_ie_len,
				DOT11_MNG_SSID_ID)) != NULL) {
				//bcm_format_ssid(ssidbuf,ssid_ie->data,ssid_ie->len);
				WL_ERR(("Rx PROB REQ %d from " MACDBG " ssid %d on P2P %d,rssi %d\n",mgmt_frame_len,
					MAC2STRDBG(hdr->sa.octet),ssid_ie->len,discover_cfgdev(cfgdev, cfg),ntoh32(rxframe->rssi)));
			} else {
				WL_ERR(("Rx PROB REQ %d from " MACDBG " ssid NULL on P2P %d,rssi %d\n",mgmt_frame_len,
					MAC2STRDBG(hdr->sa.octet),discover_cfgdev(cfgdev, cfg),ntoh32(rxframe->rssi)));
			}
		} else {
#endif /* WSS */
...
#ifdef WSS
	cfg80211_rx_mgmt(cfgdev, freq, ntoh32(rxframe->rssi),  mgmt_frame, mgmt_frame_len, 0);
#else
	cfg80211_rx_mgmt(cfgdev, freq, 0,  mgmt_frame, mgmt_frame_len, 0);
#endif /* WSS */
#elif(LINUX_VERSION_CODE >= KERNEL_VERSION(3, 13, 0))
#ifdef WSS
	cfg80211_rx_mgmt(cfgdev, freq, ntoh32(rxframe->rssi),  mgmt_frame, mgmt_frame_len, 0, GFP_ATOMIC);
#else
	...
	defined(WL_COMPAT_WIRELESS)
#ifdef WSS
	cfg80211_rx_mgmt(cfgdev, freq, ntoh32(rxframe->rssi), mgmt_frame, mgmt_frame_len, GFP_ATOMIC);
#else
...
#elif defined(CONFIG_WLAN_STR_SUPPORT)  /* STATIC_WL_PRIV_STRUCT */
	cfg->escan_info.escan_buf = kzalloc(ESCAN_BUF_SIZE, GFP_KERNEL);
	if (unlikely(!cfg->escan_info.escan_buf)) {
		WL_ERR(("cfg->escan_info.escan_buf alloc failed\n"));
		goto init_priv_mem_out;
	}
#endif /* CONFIG_WLAN_STR_SUPPORT */
...
static s32 wl_create_event_handler(struct bcm_cfg80211 *cfg)
{
	...
	WL_DBG(("Enter \n"));
        ...
        if(cfg->event_tsk.thr_pid > 0) {
                WL_ERR(("Thread is already created with id %ld.\n",cfg->event_tsk.thr_pid));
                return ret;
        }
			...
			if (iter->ndev) {
				if (iter->pm_restore && iter->pm) {
...
#else
					{
						s32 pml = PM_OFF;
						err = wldev_ioctl(iter->ndev,
							WLC_SET_PM, &pml, sizeof(iter->pm), true);
					}
#endif
					if (unlikely(err)) {
						if (err == -ENODEV)
...
eventmsg_out:
	mutex_unlock(&cfg->event_sync);
	WL_DBG(("Set event_msgs ndev=%p event=%d add=%d, err=%d \n",
			ndev->name, event, add, err));
	return err;
}
...
int wl_android_wbtext_weight_config(struct net_device *ndev, char *data,
...
int wl_android_wbtext_table_config(struct net_device *ndev, char *data,
	char *command, int total_len)
{
	struct bcm_cfg80211 *cfg = g_bcm_cfg;
