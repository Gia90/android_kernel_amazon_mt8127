...
extern atomic_t in_dbus_suspend;
#endif
...
#ifdef CONFIG_WLAN_STR_SUPPORT
        if(atomic_read(&in_dbus_suspend) && (strncmp(prot->buf, "wowl", 4) == 0)) {
                DHD_ERROR(("%s don't set wake lock in STR suspend when cmd is %s.\n", __FUNCTION__, prot->buf));
        } else {
