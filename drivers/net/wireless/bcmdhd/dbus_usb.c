...
static void dbus_usb_netdev_wl(void *handle, int idx, int cmd);
static void dbus_usb_netdev_running(void *handle, int idx);
static void dbus_usb_set_wow(void *handle, int idx, int val);
#endif
/* OS specific */
...
#ifdef CONFIG_WLAN_STR_SUPPORT
static void
dbus_usb_netdev_running(void *handle, int idx)
{
        usb_info_t *usb_info = (usb_info_t *) handle;
        ...
        if (usb_info->cbs && usb_info->cbs->netdev_running) {
                usb_info->cbs->netdev_running(usb_info->cbarg, idx);
        }
}

static void
dbus_usb_netdev_wl(void *handle, int idx, int cmd)
{
        usb_info_t *usb_info = (usb_info_t *) handle;
        ...
        if (usb_info->cbs && usb_info->cbs->netdev_wl) {
                usb_info->cbs->netdev_wl(usb_info->cbarg, idx, cmd);
        }
}

static void
dbus_usb_set_wow(void *handle, int idx, int val)
{
        usb_info_t *usb_info = (usb_info_t *) handle;
        ...
        if (usb_info->cbs && usb_info->cbs->set_wow) {
                usb_info->cbs->set_wow(usb_info->cbarg, idx, val);
        }
}
#endif
