...
/* Control fw roaming */
#if defined(OEM_ANDROID) && !defined(DISABLE_BUILTIN_ROAM)
...
#if defined(PKT_FILTER_SUPPORT) && !defined(GAN_LITE_NAT_KEEPALIVE_FILTER) && !defined(CONFIG_WLAN_STR_SUPPORT) && !defined(OEM_ANDROID)
static bool
...
#endif /* PKT_FILTER_SUPPORT && !GAN_LITE_NAT_KEEPALIVE_FILTER && !CONFIG_WLAN_STR_SUPPORT && !OEM_ANDROID */
...
{
#ifdef PKT_FILTER_SUPPORT
#if !defined(OEM_ANDROID) && !defined(CONFIG_WLAN_STR_SUPPORT) /* Packet filter conflicts with Wake-on-Wireless LAN */
	int i;
...
#endif /* !OEM_ANDROID and !CONFIG_WLAN_STR_SUPPORT */
#endif /* PKT_FILTER_SUPPORT */
}
...
#ifdef CONFIG_WLAN_STR_SUPPORT
static void
dhd_dbus_netdev_running(void *handle, int idx)
{
        dhd_info_t *dhd = (dhd_info_t *)handle;
        if (dhd && dhd->iflist[idx]) {
                rtmsg_ifinfo(RTM_NEWLINK, dhd->iflist[idx]->net, IFF_RUNNING);
        }
...
static void
dhd_dbus_netdev_wl(void *handle, int idx, int cmd)
{
       s32 val = 0;
       dhd_info_t *dhd = (dhd_info_t *)handle;
        if (dhd && dhd->iflist[idx]) {
                wldev_ioctl(dhd->iflist[idx]->net, cmd, &val, sizeof(s32), true);
        }
...
void
dhd_dbus_set_suspend_state(void *handle, uint32 val)
{
	dhd_info_t *dhd = (dhd_info_t *)handle;
	if (dhd) {
		atomic_set(&in_dbus_suspend, val);
		DHD_ERROR(("%s val %d\n", __FUNCTION__, atomic_read(&in_dbus_suspend)));
	}
}

void
dhd_dbus_set_wow(void *handle, int idx, int val)
{
       s32 wowl, wowl_gpio;
       dhd_info_t *dhd = (dhd_info_t *)handle;
       ...
       if(dhd && dhd->iflist[idx]) {
              if(val) {
                     //dhd_wl_ioctl_set_intiovar(&dhd->pub,"wowl_pattern", , WLC_SET_VAR, TRUE,0);
                     dhd_wl_ioctl_set_intiovar(&dhd->pub,"wowl_activate", 1, WLC_SET_VAR, TRUE, 0);

                     dhd_wl_ioctl_get_intiovar(&dhd->pub,"wowl", &wowl, WLC_GET_VAR, FALSE, 0);
                     dhd_wl_ioctl_get_intiovar(&dhd->pub,"wowl_gpio", &wowl_gpio, WLC_GET_VAR, FALSE, 0);
                     DHD_ERROR(("%s: wowl %d wowl_gpio %d\n", __FUNCTION__, wowl, wowl_gpio));
              } else {
                     dhd_wl_ioctl_set_intiovar(&dhd->pub,"wowl_clear", 1, WLC_SET_VAR, TRUE, 0);
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	int arpoe = 1;
#endif
#if defined(CONFIG_WLAN_STR_SUPPORT)
	int wowl = 1;
#endif
...
#ifdef DISABLE_BEAMFORMING
	/* need to put txbf configuration in the early init */
	bcm_mkiovar("txbf", (char *)&txbf, 4, iovbuf, sizeof(iovbuf));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
		sizeof(iovbuf), TRUE, 0)) < 0) {
		DHD_ERROR(("%s Reset txbf failed  %d\n", __FUNCTION__, ret));
	}
	bcm_mkiovar("txbf_bfr_cap", (char *)&txbf, 4, iovbuf, sizeof(iovbuf));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
		sizeof(iovbuf), TRUE, 0)) < 0) {
		DHD_ERROR(("%s Reset txbf_bfr_cap failed  %d\n", __FUNCTION__, ret));
	}
	bcm_mkiovar("txbf_bfe_cap", (char *)&txbf, 4, iovbuf, sizeof(iovbuf));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf,
		sizeof(iovbuf), TRUE, 0)) < 0) {
		DHD_ERROR(("%s Reset txbf_bfe_cap failed  %d\n", __FUNCTION__, ret));
	}
#endif /* DISABLE_BEAMFORMING */
...
#if defined(DISABLE_11H_SOFTAP)
	/* To set spect, wl must be down */
	if (dhd_wl_ioctl_cmd(dhd, WLC_DOWN, NULL, 0, TRUE, 0) < 0) {
		DHD_ERROR(("%s: wl down failed\n", __FUNCTION__));
	}
	ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_SPECT_MANAGMENT, &spect, sizeof(s32), TRUE, 0);
	if (ret < 0) {
		DHD_ERROR(("%s: wl spect 0 failed - %d\n", __FUNCTION__, ret));
	}
	if (dhd_wl_ioctl_cmd(dhd, WLC_UP, NULL, 0, TRUE, 0) < 0) {
		DHD_ERROR(("%s: wl up failed\n", __FUNCTION__));
	}
...
#ifdef WL_CFG80211
// fosmod_wifi_memory begin
#ifdef PANIC_IF_PROBE_FAIL
	if (dhd->dhd_state & DHD_ATTACH_STATE_DONE)
#endif
// fosmod_wifi_memory end
	wl_cfg80211_down(NULL);
#endif /* WL_CFG80211 */
...
static void *
__dhd_dbus_probe_cb__(void *arg, const char *desc, uint32 bustype, uint32 hdrlen)
#else
void *
...
// fosmod_wifi_memory begin
#ifdef PANIC_IF_PROBE_FAIL
#define NUM_OF_PROBE_TRIES  2
#define PANIC_PROG  "/system/etc/wifi_log_levels.sh"

void *dhd_dbus_probe_cb(void *arg, const char *desc, uint32 bustype, uint32 hdrlen)
{
	int i, r;
	void *ptr;
	char *argv[] = {PANIC_PROG, "panic", NULL };
	char *envp[] = {"HOME=/", "TERM=linux", "PATH=/system/bin:/system/xbin", NULL};

	for (i=1; i<=NUM_OF_PROBE_TRIES; i++) {
		ptr = __dhd_dbus_probe_cb__(arg, desc, bustype, hdrlen);
		if (ptr) {
			return ptr;
		}
		DHD_ERROR(("%s: Loading wifi driver failed. Try count=%d\n", __FUNCTION__, i));
	}

	DHD_ERROR(("dhd_dbus_probe_cb(): Loading wifi driver failed %d times.", NUM_OF_PROBE_TRIES));
	r = call_usermodehelper(PANIC_PROG, argv, envp, UMH_WAIT_EXEC);
	DHD_ERROR(("dhd_dbus_probe_cb(): called %s for panic. return=%d", PANIC_PROG, r));
	return NULL;
}
#endif
// fosmod_wifi_memory end
